...
import {
  Package, User as UserIcon, Plus, Clock, CheckCircle, Navigation,
  PackageCheck, Truck, X, Radar, Camera,
  ChevronRight, ShieldCheck, FileText,
  Receipt, Check, Ban, Database, Search, Loader2
} from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  // EXTRA: refresh bookings whenever the tab gains focus (helps after returning from checkout)
  useEffect(() => {
    const onFocus = () => {
      const saved = localStorage.getItem('droppit_bookings_db');
      if (!saved) return;
              ...
              <div className="space-y-2">
                ...
                {!hasPickup ? (
                  ...
                  <div className="relative rounded-2xl overflow-hidden border border-lime-400/50 bg-slate-950 p-4 flex items-center justify-center shadow-inner">
                    ...
                    <div className="absolute top-2 right-2 bg-lime-400 p-1 rounded-full">
  ...
  const renderAdminView = () => (
    <div className="space-y-6 animate-in fade-in duration-500 pb-24 font-inter">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="bg-slate-900 text-[8px] font-black uppercase px-2.5 py-1 rounded border border-slate-800 text-slate-500 leading-none">
          Total Shipments: {bookings.length}
        </span>
      </div>

      <div className="space-y-4">
        {bookings.length === 0 ? (
          <div className="bg-slate-900/20 border border-dashed border-slate-800 rounded-[2.5rem] p-16 text-center text-slate-800 text-[10px] font-black uppercase tracking-[0.4em] italic leading-loose">
            No deliveries found
          </div>
        ) : (
          bookings.map(b => (
            <div key={b.id} className="bg-slate-900 border border-slate-800 rounded-3xl p-5 space-y-4 shadow-xl group hover:border-lime-400/30 transition-all relative">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[8px] bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded font-black uppercase tracking-widest italic leading-none">
                    ID: {b.id.slice(0, 8)}
                  </span>
                  <h4 className="text-sm font-black text-white mt-1 uppercase italic tracking-tighter leading-tight">
                    {b.carrier} Order
                  </h4>
                </div>
                <div
                  className={`text-[8px] font-black px-2.5 py-1 rounded uppercase italic leading-none ${
                    b.status === 'completed'
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  { label: 'Asset', url: (b as any).qr_url || (b as any).label_url, icon: FileText },
                  { label: 'Pickup', url: (b as any).pickup_proof_url, icon: Camera },
                  { label: 'Receipt', url: (b as any).dropoff_receipt_url, icon: Receipt },
                ].map((art, i) => (
                  <div
                    key={i}
                    className={`aspect-square bg-slate-950 rounded-xl border flex flex-col items-center justify-center transition-all ${
                      art.url ? 'border-lime-400/40' : 'border-slate-800'
                    }`}
                  >
                    {art.url ? (
                      <div className="text-center p-1">
                        <art.icon size={12} className="text-lime-400 mx-auto" />
                        <p className="text-[4px] text-lime-400/40 font-black uppercase tracking-tighter truncate w-10 mt-1">
                          {art.url}
                        </p>
                      </div>
                    ) : (
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => updateBooking(b.id, { status: 'issue_hold' })}
                  className="flex-1 bg-orange-500/10 border border-orange-500/30 text-orange-500 text-[8px] font-black uppercase py-2.5 rounded-lg active:scale-95 leading-none"
                >
                  Hold
                </button>
                <button
                  onClick={() => updateBooking(b.id, { courier_id: null as any, status: 'booked' })}
                  className="flex-1 bg-blue-500/10 border border-blue-500/30 text-blue-500 text-[8px] font-black uppercase py-2.5 rounded-lg active:scale-95 leading-none"
                >
                  Reset
                </button>
                <button
                  onClick={() => updateBooking(b.id, { status: 'canceled' })}
                  className="flex-1 bg-red-500/10 border border-red-500/30 text-red-500 text-[8px] font-black uppercase py-2.5 rounded-lg active:scale-95 leading-none"
                >
                  Cancel
                </button>
              </div>
    ...
    <div className="max-w-md mx-auto min-h-screen bg-slate-950 pb-24 text-slate-100 font-inter">
      <div className="p-6 flex justify-between items-center bg-slate-950 sticky top-0 z-40 border-b border-slate-900/50 backdrop-blur-md">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold tracking-tighter text-xl italic text-white uppercase leading-none">
          ...
          <>
              ...
              <div className="space-y-8 animate-in fade-in duration-500">
                <div className="bg-gradient-to-br from-lime-400 to-lime-500 rounded-[2.5rem] p-8 shadow-xl shadow-lime-400/20 relative overflow-hidden group">
                  ...
                  <div className="relative z-10">
                    <h3 className="text-slate-950 text-4xl font-black mb-1 uppercase tracking-tighter italic leading-none">
                      Drop packages,<br />not your plans.
                    </h3>
                    <p className="text-slate-950/60 text-[11px] font-black uppercase tracking-[0.2em] mb-8 italic leading-tight">
                      Fast, secure package pickup service
                    </p>
                    <button
                      onClick={() => setIsBooking(true)}
                      className="flex items-center gap-3 bg-slate-950 text-white font-black py-4 px-8 rounded-2xl active:scale-95 transition-all uppercase italic tracking-tighter text-sm shadow-2xl hover:bg-slate-900 leading-none"
                    >
                ...
                <div className="space-y-4">
                  ...
                  {activeJobs.length === 0 ? (
                    <div className="bg-slate-900/20 border border-dashed border-slate-800 rounded-[2.5rem] p-16 text-center text-slate-800 uppercase text-[9px] font-black tracking-[0.4em] italic leading-relaxed">
                      ...
                      {activeJobs.map(job => (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest leading-none mb-1 italic">
                                {job.carrier} Order
                              </p>
                              <h4 className="font-black text-sm uppercase italic text-white tracking-tighter leading-tight">
                                {job.dropoff_name}
                              </h4>
                            </div>
                          </div>
                ...
                <div className="space-y-4">
                  ...
                  {userHistory.length === 0 ? (
                    <div className="text-center text-slate-800 text-[8px] font-black uppercase tracking-widest p-8 italic leading-none">
                      History is empty.
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {userHistory.map(job => (
                        <div
                          key={job.id}
                          className="bg-slate-900/40 p-5 rounded-3xl border border-slate-800 flex items-center justify-between hover:bg-slate-900/60 transition-all group"
                        >
                          <div className="flex items-center gap-4">
                            <div
                              className={`p-2 rounded-xl border transition-all leading-none ${
                                job.status === 'completed'
                            ...
                            <div>
                              <h4 className="font-black text-xs uppercase italic text-white leading-tight">
                                {job.carrier} Trip
                              </h4>
                              <p className="text-[8px] text-slate-500 font-bold uppercase tracking-widest italic mt-0.5 leading-none">
                                {job.status.replace('_', ' ')} • {job.dropoff_name}
                              </p>
                            </div>
                          </div>
                          <span className="text-[10px] font-black text-white italic tracking-tighter leading-none">
                            ${Number(job.price_total).toFixed(2)}
                          </span>
                        </div>
                      ))}
                    </div>
            ...
            {activeTab === 'tracking' && (
              <div className="space-y-6 animate-in fade-in duration-500 pb-12">
                ...
                {activeJobs.length === 0 ? (
                  <div className="p-20 text-center text-slate-800 text-[10px] font-black uppercase italic tracking-[0.4em] leading-loose">
                    ...
                    {activeJobs.map(job => (
                      <div key={job.id} className="bg-slate-900 border border-lime-400/20 rounded-[2.5rem] p-8 space-y-6 shadow-2xl relative overflow-hidden group">
                        <div className="flex justify-between items-start relative z-10">
                          <div>
                            <h4 className="text-2xl font-black italic uppercase tracking-tighter text-white leading-none">
                              {job.carrier} SHIPMENT
                            </h4>
                            <p className="text-[10px] text-lime-400 font-black uppercase tracking-widest mt-2 italic leading-none">
                              {job.status.replace('_', ' ')}
                            </p>
                          </div>
                          <div className="bg-slate-800 p-3 rounded-2xl text-lime-400 border border-slate-700 shadow-xl leading-none">
                            <Radar className="animate-pulse" size={24} />
                          </div>
                        </div>

                        <div className="grid grid-cols-2 gap-3 relative z-10">
                          <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3 group/art shadow-inner">
                            <span className="text-[7px] font-black uppercase text-slate-600 tracking-widest italic block leading-none">
                              QR/Label
                            </span>
                            <div className="h-24 bg-slate-900/50 rounded-xl border border-lime-400/20 flex flex-col items-center justify-center p-2 leading-none">
                              ...
                              <p className="text-[4px] text-slate-700 font-black uppercase truncate w-full text-center tracking-tighter italic leading-none">
                                {(job as any).qr_url || (job as any).label_url}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3 group/art shadow-inner">
                            ...
                            {(job as any).pickup_proof_url ? (
                              <div className="h-24 bg-slate-900/50 rounded-xl border border-lime-400/20 flex flex-col items-center justify-center p-2 leading-none">
                                <Camera size={20} className="text-lime-400 mb-1.5" />
                                <p className="text-[4px] text-slate-700 font-black uppercase truncate w-full text-center tracking-tighter leading-none italic">
                                  {(job as any).pickup_proof_url}
                                </p>
                              </div>
                            ) : (
                              <div className="h-24 bg-slate-900/50 rounded-xl border border-dashed border-slate-800 flex items-center justify-center text-[7px] font-black text-slate-800 italic uppercase leading-none">
                                Waiting for pickup
                              </div>
                            )}
                          </div>
                        ...
                        {(job as any).dropoff_receipt_url && (
                          <div className="bg-slate-950 p-5 rounded-3xl border border-lime-400/40 space-y-4 animate-in zoom-in-95 duration-700 shadow-2xl relative z-10">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[9px] font-black uppercase text-lime-400 italic tracking-[0.2em] leading-none">
                                Delivery Receipt
                              </span>
                            </div>
                            <div className="h-12 bg-slate-900/50 rounded-xl border border-lime-400/20 flex items-center justify-center px-4 leading-none">
                              ...
                              <p className="text-[6px] text-lime-400/60 font-black uppercase italic tracking-widest truncate w-full text-center leading-none">
                                {(job as any).dropoff_receipt_url}
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="flex justify-between items-center z-10 relative">
                          <span className="text-[7px] text-slate-600 font-black uppercase tracking-widest italic leading-none">
                            Real-time status: {job.status.replace('_', ' ')}
                          </span>
                          <span className="text-[7px] text-slate-600 font-black uppercase tracking-widest italic leading-none">
                            SECURE DATA
                          </span>
        ...
        <div className="fixed inset-0 z-[60] bg-slate-950 flex flex-col overflow-y-auto">
          <div className="max-w-md mx-auto w-full min-h-screen">
            <BookingWizard
              ...
              // NEW: booking details → payment page
              onGoToCheckout={(bookingDraft: any, amountCents: number) => {
                setIsBooking(false);
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-slate-950/80 backdrop-blur-2xl border-t border-slate-900/50 z-50">
        <div className="max-w-md mx-auto flex justify-between px-10 items-center">
          <button
            ...
            className={`flex flex-col items-center gap-1.5 transition-all duration-300 leading-none ${
              activeTab === 'deliveries' ? 'text-lime-400 scale-110' : 'text-slate-700'
            }`}
          >
          ...
          <button
            onClick={() => setIsBooking(true)}
            className="bg-lime-400 p-4 rounded-2xl shadow-2xl shadow-lime-400/30 text-slate-950 active:scale-90 transition-all border-4 border-slate-950 -mt-10 hover:rotate-3 leading-none"
          >
            <Plus size={28} />
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1.5 transition-all duration-300 leading-none ${
              activeTab === 'tracking' ? 'text-lime-400 scale-110' : 'text-slate-700'
            }`}
          >
          ...
          {user.role === 'admin' && (
            <button
              ...
              className={`flex flex-col items-center gap-1.5 transition-all duration-300 leading-none ${
                activeTab === 'admin' ? 'text-lime-400 scale-110' : 'text-slate-700'
              }`}
            >
